...
static Value eval_index(Interpreter* interp, ASTNode* node) {
    Value object = evaluate(interp, node->as.index_expr.object);
    Value index = evaluate(interp, node->as.index_expr.index);
    ...
    switch (node->type) {
        ...
        case AST_OBJECT: {
            ...
            if (klass_ptr != NULL && klass_ptr->type == VAL_OBJECT && klass_ptr->as.object->ast != NULL) {
                ...
                Value obj = value_object(class_node->as.class_decl.name, env_create(interp->global_env));
                ...
                for (int i = 0; i < class_node->as.class_decl.method_count; i++) {
                    ...
                    Value m_val; m_val.type = VAL_FUNCTION; m_val.as.function = fn;
        ...
        case AST_INDEX_SET: {
            Value obj = evaluate(interp, node->as.index_set.object);
            Value idx = evaluate(interp, node->as.index_set.index);
            Value val = evaluate(interp, node->as.index_set.value);
        ...
        case AST_GET: {
            Value obj = evaluate(interp, node->as.get_expr.object);
            if (obj.type == VAL_MAP) {
        ...
        case AST_SET: {
            Value obj = evaluate(interp, node->as.set_expr.object);
            Value val = evaluate(interp, node->as.set_expr.value);
    ...
    switch (node->type) {
        ...
        case AST_IMPORT: {
            ...
            if (source) {
                ...
                free(source);
                // Lexer/Parser/Program free skipped for brevity in small runtime
            }
            return;
        }
        
        case AST_CLASS: {
            Value class_val = value_object(node->as.class_decl.name, env_create(interp->global_env));
        ...
        case AST_WHEN: {
            Value cond = evaluate(interp, node->as.when_stmt.condition);
